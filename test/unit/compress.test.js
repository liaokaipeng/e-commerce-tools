...
async function run() {
  // ===== 选项归一化 =====
  {
    ...
    t('normalizeOpts 缺省取 DEFAULTS', d.maxMB === DEFAULTS.maxMB && d.maxSeconds === DEFAULTS.maxSeconds
      && d.overMode === DEFAULTS.overMode && d.maxLongSide === DEFAULTS.maxLongSide);
    const n = normalizeOpts({ maxMB: 'abc', maxSeconds: null, overMode: '乱写', maxLongSide: -5 });
    t('normalizeOpts 非法值回落默认且 mode 只认 trim/speed',
      n.maxMB === DEFAULTS.maxMB && n.maxSeconds === DEFAULTS.maxSeconds
      && n.overMode === 'trim' && n.maxLongSide === 0, JSON.stringify(n));
    ...
    t('normalizeOpts 接受 speed 模式', normalizeOpts({ overMode: 'speed' }).overMode === 'speed');
  }
  ...
  // ===== 码率计算（trim 模式）=====
  {
    const opts = normalizeOpts({ maxMB: 30, maxSeconds: 60, overMode: 'trim', maxLongSide: 1280 });
    const p = planFor(bigInfo, opts);
    t('planFor trim：输出时长截到 60 秒', p.outDuration === 60, String(p.outDuration));
    t('planFor trim：不带变速与音频滤镜', p.speedRatio === 1 && p.audioFilter === '');
    // 目标 30MB × 0.96 余量 × 8 ÷ 60 秒 ≈ 4026kbps，减去 96k 音频 ≈ 3930
    t('planFor trim：视频码率按预算算出（30MB/60s 约 3930kbps）',
  ...
  // ===== 变速模式 =====
  {
    const opts = normalizeOpts({ maxSeconds: 60, overMode: 'speed' });
    const p = planFor(bigInfo, opts);
    t('planFor speed：倍速 = 原时长 / 上限', Math.abs(p.speedRatio - 3) < 1e-9, String(p.speedRatio));
    t('planFor speed：输出时长等于上限', p.outDuration === 60);
    t('planFor speed：3 倍速拆成 atempo=2 + atempo=1.5', p.audioFilter === 'atempo=2,atempo=1.5000', p.audioFilter);

    const short = planFor({ size: 100 * MB, duration: 40, width: 1920, height: 1080, hasAudio: true }, opts);
    t('planFor speed：时长本身没超则不加速（只压体积）',
  ...
  // ===== 命令行拼装 =====
  {
    const plan = planFor(bigInfo, normalizeOpts({}));
    const args = buildArgs({ input: 'in.mp4', output: 'out.mp4', plan });
    const has = (a) => args.includes(a);
    t('buildArgs 输出截断到 60 秒', has('-t') && args[args.indexOf('-t') + 1] === '60', args.join(' '));
    ...
    t('buildArgs 缩放滤镜生效', has('-vf') && args[args.indexOf('-vf') + 1] === 'scale=1280:-2');
    ...
    const speedPlan = planFor(bigInfo, normalizeOpts({ overMode: 'speed' }));
    const speedArgs = buildArgs({ input: 'i', output: 'o', plan: speedPlan });
    t('buildArgs 变速模式同时给 setpts 与 atempo',
      speedArgs.join(' ').includes('setpts=PTS/3.000000') && speedArgs.includes('-af')
      && speedArgs[speedArgs.indexOf('-af') + 1] === 'atempo=2,atempo=1.5000', speedArgs.join(' '));
  }

  // ===== 产物命名 =====
  {
    t('outputName 加后缀并统一转 mp4', outputName('a/b/视频名.mov') === '视频名_compressed.mp4', outputName('a/b/视频名.mov'));
    t('isOwnOutput 识别上次的产物', isOwnOutput('x_compressed.mp4') === true);
  ...
  // ===== 递归扫描 =====
  {
    ...
    fs.writeFileSync(path.join(own, 'd.mp4'), 'x');       // 产物目录：跳过
    fs.writeFileSync(path.join(hidden, 'e.mp4'), 'x');    // 隐藏目录：跳过
    fs.writeFileSync(path.join(dir, 'f_compressed.mp4'), 'x'); // 上次产物：跳过
