...
//   2. 开放平台刷新链路（test/openapi-refresh.test.js）：离线 mock 网关，不启动服务、不触网
//   3. 接口冒烟测试（test/api.test.js）：临时端口 8865，验页面与 API 路由，
...
async function main() {
  ...
  console.log('\n===== 开放平台刷新链路（离线 mock 网关） =====');
  await require('./test/openapi-refresh.test').run();
  console.log('\n===== 接口冒烟测试 =====');
