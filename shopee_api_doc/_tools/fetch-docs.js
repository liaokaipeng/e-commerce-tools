...
function renderGuideListZh(enList, cnList) {
  const officialZh = collectZhNames(cnList);
  const out = [];
  out.push('# Shopee Open Platform 开发者指南目录（中文版）');
  out.push('');
  out.push('> 数据来源：`/developer_guide/list`（en / zh-Hans），抓取于 ' + new Date().toISOString().slice(0, 10) + '。');
  ...
  out.push('- 「编号」即文章 document_id，可直接拼出官方页面链接：`/developer-guide/{编号}`。');
  out.push('');
  ...
  // 文章表格
  const articleTable = (items, noteMap) => {
    out.push('| 中文标题 | 原文标题 | 编号 | 官方页面 |');
    out.push('|---|---|---|---|');
    for (const it of items) {
      ...
      out.push(
        '| ' + zh.name + mark + ' | ' + enName + ' | ' + it.item_id +
          ' | [EN](' + SG + '/developer-guide/' + it.item_id + ') ｜ [中文](' + CN + '/developer-guide/' + it.item_id + ') |'
      );
    }
    out.push('');
  };
...
function renderApiCatalogZh(tree) {
  const out = [];
  out.push('# Shopee Open Platform API 参考目录（v2，中文版）');
  out.push('');
  const totalItems = tree.modules.reduce((s, m) => s + (m.items || []).length, 0);
  out.push('> 数据来源：`/doc/module?version=2`，抓取于 ' + new Date().toISOString().slice(0, 10) + '，共 ' + tree.modules.length + ' 个模块、' + totalItems + ' 个条目。');
  out.push('> 官方页面：<https://open.shopee.cn/documents>');
  out.push('');
  out.push('说明：');
  out.push('');
  out.push('- 接口名保持官方标识符（如 `v2.ams.get_open_campaign_added_product`）不变，仅模块名给出中文译名。');
  out.push('- 「指南总览」模块下为文档指南页（非 API），官方跳转链接形如 `/developer-guide?from=doc&id={条目编号}`。');
  out.push('- 接口官方页面 URL 形如 `/documents/v2/{api_name}?module={module_id}&type=1`。');
  out.push('');

  // 模块总览
  out.push('## 模块总览');
  out.push('');
  out.push('| 模块 | 中文名称 | 条目数 |');
  out.push('|---|---|---|');
  for (const mod of tree.modules) {
    const zh = ZH_MODULE[mod.module_id] || mod.module_name;
    out.push('| [`' + mod.module_name + '`](#' + slugAnchor(zh + '（' + mod.module_name + '）') + ') | ' + zh + ' | ' + (mod.items || []).length + ' |');
  }
  out.push('');
  ...
  for (const mod of tree.modules) {
    ...
    if (mod.type === 1) {
      ...
      for (const it of mod.items || []) {
        out.push(
          '| `' + it.name + '` | [页面](' + CN + '/documents/v2/' + encodeURIComponent(it.name) + '?module=' + mod.module_id + '&type=1) |'
        );
      }
    } else {
      out.push('| 中文名称 | 官方名称 | 条目编号 | 官方跳转 |');
      out.push('|---|---|---|---|');
      for (const it of mod.items || []) {
        const zhName = ZH_OVERVIEW_ITEM[it.id] || it.name;
        out.push(
          '| ' + zhName + ' | ' + it.name + ' | ' + it.id +
            ' | [跳转](' + SG + '/developer-guide?from=doc&id=' + it.id + ') |'
        );
      }
    }
    out.push('');
...
async function main() {
  ...
  // 1) 开发者指南目录（中英）
  const guideListEn = await fetchJson(SG + '/opservice/api/v1/developer_guide/list?language_code=en', SG);
  const guideListCn = await fetchJson(CN + '/opservice/api/v1/developer_guide/list?language_code=zh-Hans', CN);
  saveJson(path.join(RAW_DIR, 'guide_list_en.json'), guideListEn);
  saveJson(path.join(RAW_DIR, 'guide_list_cn.json'), guideListCn);

  // 2) API 模块树（v2）
  const moduleTree = await fetchJson(CN + '/opservice/api/v1/doc/module/?version=2', CN);
  saveJson(path.join(RAW_DIR, 'doc_module_v2.json'), moduleTree);

  // 3) 渲染两份中文目录
  const guideDir = path.join(ROOT, 'developer-guide');
  fs.mkdirSync(guideDir, { recursive: true });
  fs.writeFileSync(path.join(guideDir, 'README.md'), renderGuideListZh(guideListEn, guideListCn), 'utf8');
  console.log('[doc] developer-guide/README.md');

  const apiDir = path.join(ROOT, 'api');
  fs.mkdirSync(apiDir, { recursive: true });
  fs.writeFileSync(path.join(apiDir, 'README.md'), renderApiCatalogZh(moduleTree), 'utf8');
